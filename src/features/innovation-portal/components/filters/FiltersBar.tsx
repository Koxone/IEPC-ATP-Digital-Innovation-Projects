    ...
    <section className="surface-card flex flex-col gap-4 p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3 text-xs text-ford-text-muted">
          <span>
            <span className="font-semibold text-white tabular-nums">{totalShown}</span>
            <span className="text-ford-text-dim"> / {totalProjects}</span> initiatives shown
          </span>
          {hasActive && (
            <button
              type="button"
              onClick={clear}
              className="inline-flex items-center gap-1 rounded-md border border-ford-border bg-ford-surface px-2 py-1 text-[11px] font-semibold text-ford-text-muted transition hover:border-ford-border-strong hover:text-white"
            >
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <label className="flex flex-col gap-1.5 text-xs">
          ...
          <span className="relative">
            <Search className="pointer-events-none absolute top-1/2 left-2.5 h-3.5 w-3.5 -translate-y-1/2 text-ford-text-dim" aria-hidden />
            <input
              type="text"
  ...
  return (
    <label className="flex flex-col gap-1.5 text-xs">
      ...
      <select
        ...
        className="input-base appearance-none px-3 py-2 text-sm"
      >
        {options.map((opt) => (
